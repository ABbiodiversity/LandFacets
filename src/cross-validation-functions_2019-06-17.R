#
# Title: Functions to execute leav one out cross validation of models
# Created: June 17th, 2019
# Last Updated: June 17th, 2019
# Author: Brandon Allen
# Objectives: Soil and land facet habitat model functions
# Keywords: Southern models, Site predictions
#
...
cv_southern_models <- function (data.analysis, results.store, landscape.models, prediction.matrix, species.ID) {
        ...
        # If any coefficient happens to be fixed at 1 or 0, make slight modification so Inf errors are not thrown.
        results.store$landscape.coef[species.ID, ][results.store$landscape.coef[species.ID, ] == 0] <- 0.0001
        results.store$landscape.coef[species.ID, ][results.store$landscape.coef[species.ID, ] == 1] <- 0.9999
...
cv_site_predictions <- function(model.coef, landscape.data, climate.data, paspen.data, species.data, species.ID) {
        ...
        predict.out <- matrix(c(auc.obs, auc.predict), nrow = species.data[1, "nQuadrant"], 
                              ncol = 2, byrow = FALSE, 
                              dimnames = list(NULL, c(paste(species.ID, "obs", sep = "_"), paste(species.ID, "pred", sep = "_"))))
        
        return(predict.out)
        
        rm(auc.obs, auc.predict, predict.out, veg.prediction, climate.prediction, aspen.prediction, site.prediction)
...
cv_auc <- function(soil.matrix, facet.matrix, obs, pred, species.ID) {
